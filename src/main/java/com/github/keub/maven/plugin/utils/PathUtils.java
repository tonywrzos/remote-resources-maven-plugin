...
public class PathUtils {
	...
	public static final String MULTIPLE_SLASH = "//";
	...
	public static void addEndingSlashIfNeeded(StringBuilder src) {
		...
		if (src != null && src.charAt(src.length() - 1) != File.separator.charAt(0)) {
	...
	public static File getWorkspace(CopyResourcesMojo copyResourcesMojo) {
		...
		StringBuilder retval = new StringBuilder(copyResourcesMojo.getProject().getBuild().getDirectory());
	 ...
	 * @return
	 */
	public static String getAbsoluteProjectPath(Project project, String rootLocation) {
		StringBuilder retval = new StringBuilder(rootLocation);
		PathUtils.addEndingSlashIfNeeded(retval);
		retval.append(GitHelper.extractRepositoryNameFromUrl(String.valueOf(project.getUri())));
		PathUtils.addEndingSlashIfNeeded(retval);
